 ...
 * wait_ctl_reply                            
   发送者检查是否另一个TCP连接已经请求，如果是，那么处理重复，当收到Start-Control-Connection-Reply，效验兼容版本，如果版本正确，发送者移动到建立状态，如果版本不支持，发起者发送一个Stop-Control-Connection-Request请求。并将发送者至移动到wait_stop_reply            
 * established                            
...
```
 * idle
   控制连接接收器等待一个开放的TCP端口1723,当TCP链接收到通知，它准备接收PPTP消息，当收到Start-Control-Connection-Request，检查版本是否兼容，然后回复一个Start-Control-Connection-Request-Reply,并进入established状态，如果不支持，接收器发送一个Start-Control-Connection-Reply,关闭TCP连接，并保持空闲状态。
 * established
