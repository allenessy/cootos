 ...
 * wait_ctl_reply                            
   发送者检查是否另一个TCP连接已经请求，如果是，那么处理重复，当收到Start-Control-Connection-Reply，效验兼容版本，如果版本正确，发送者移动到建立状态，如果版本不支持，发起者发送一个Stop-Control-Connection-Request请求。并将发送者至移动到wait_ctl_reply            
 * established                            
